    ...
    await transporter.sendMail({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; color: #333;">
          <h2 style="color: #047857;">Verification Code</h2>
          <p>Use the code below to verify your account:</p>
          <h1 style="color: #047857; background: #f0fdf4; padding: 10px; display: inline-block; border-radius: 8px;">${code}</h1>
...
router.post('/forgot-password/reset', async (req, res) => {
  ...
  await supabase
    .from('users')
    .update({ password: new_password, otp_code: null, otp_expires_at: null })
    .eq('id', user.id);
...
router.post('/register', async (req, res) => {
  ...
  if (exist) {
    if (exist.verified) {
      ...
      await sendOtpMail(email, otp_code);
      return res.json({ message: 'Account exists but unverified. OTP resent.' });
    }
  }
  ...
  try {
    ...
    return res.status(500).json({ error: 'ID generation failed.' });
  }

  // Insert New User (REMOVED OLD BALANCE COLUMNS)
  ...
  // --- NEW STEP: Create Wallet for User ---
...
router.put('/:user_id', async (req, res) => {
  const supabase = req.supabase;
  const { user_id } = req.params;
  const { data, error } = await supabase.from('users').update(req.body).eq('id', user_id).select().single();
...
router.post('/change-password', async (req, res) => {
  ...
  await supabase.from('users').update({ password: new_password }).eq('id', user_id);
  res.json({ message: 'Password changed' });
});

// -------- Get Profile by Short ID (UPDATED) --------
router.get('/profile', async (req, res) => {
  ...
  try {
    ...
    // FIX: Fetch wallet from 'wallets' table
    ...
    const wallet = {
      ...
      // Legacy support for frontend if needed, otherwise just send balance
      usdt: 0,
      alipay: 0,
      wechat: 0
    };
