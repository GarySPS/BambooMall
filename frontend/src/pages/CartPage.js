...
import { FaStore, FaBoxOpen, FaReceipt, FaSpinner, FaLayerGroup } from "react-icons/fa";
...
export default function CartPage() {
  ...
  const handleRefund = async (orderId) => {
    try {
      ...
      // Refresh list
      setLoading(true);
      fetchCartOrders(userId).then((data) => {
        setCartOrders(data);
        setLoading(false);
      });
    } catch (err) {
  ...
  // Helper to calculate Projected Sales
  // Resale Price (Market Value) = Product Price * Quantity
  const getProjectedSale = (order) => {
    const qty = Number(order.quantity || 1);
    const marketPrice = Number(order.product?.price || 0);
    return marketPrice * qty;
  };

  // Helper to calculate Projected Profit
  const getProjectedProfit = (order) => {
    // If backend already calculated 'earn' (unlikely for pending, but safe check)
    if (Number(order.earn) > 0) return Number(order.earn);
    
    // Otherwise: Market Value - Cost
    const cost = Number(order.amount || 0);
    const sale = getProjectedSale(order);
    return sale - cost;
  };
        ...
        {activeOrders.length === 0 ? (
          ...
          <div className="space-y-5 pb-20">
            {activeOrders.map((order) => {
              const projectedSale = getProjectedSale(order);
              const projectedProfit = getProjectedProfit(order);
              ...
              return (
                ...
                >
                  {/* Status Badge */}
                  <div className={`absolute top-0 right-0 text-white text-[10px] font-bold px-3 py-1 rounded-bl-xl rounded-tr-xl shadow-sm z-10 uppercase tracking-wider
                    ${order.status === 'refund_pending' ? 'bg-yellow-500' : 'bg-gradient-to-l from-green-500 to-green-600'}`}>
                  ...
                  <div className="flex flex-row gap-4 mb-4 mt-2">
                    <div className="relative w-24 h-24 flex-shrink-0">
                        <img
                            ...
                            alt={order.title}
                            className="w-full h-full rounded-xl object-cover border border-gray-100 shadow-md bg-white"
                        />
                        {/* Quantity Badge on Image */}
                        <div className="absolute -bottom-2 -right-2 bg-gray-800 text-white text-xs font-bold px-2 py-1 rounded-lg shadow border border-white flex items-center gap-1">
                            <FaLayerGroup size={10} /> x{order.quantity || 1}
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col justify-between flex-1 min-w-0">
                      <div>
                        <h3 className="font-bold text-green-950 text-sm leading-snug line-clamp-2 mb-2">
                          {order.title || order.product?.title || "Product"}
                        </h3>
                  ...
                  <div className="grid grid-cols-3 gap-2 bg-green-50/50 rounded-xl p-3 border border-green-100/50 mb-4">
                    <div className="flex flex-col items-center border-r border-green-200/50">
                      <span className="text-[10px] text-gray-500 uppercase font-bold tracking-wide">Total Cost</span>
                    ...
                    <div className="flex flex-col items-center border-r border-green-200/50">
                      ...
                      <span className="text-sm font-semibold text-gray-700">
                        ${projectedSale.toFixed(2)}
                      </span>
                    </div>
                    <div className="flex flex-col items-center">
                      <span className="text-[10px] text-green-600 uppercase font-bold tracking-wide">Est. Profit</span>
